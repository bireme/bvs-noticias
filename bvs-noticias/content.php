	...
	<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
		...
		<header class="entry-header">
			...
			<h1 class="entry-title">
				<a href="<?php the_permalink(); ?>" title="<?php echo esc_attr( sprintf( _( 'Permalink to %s' ), the_title_attribute( 'echo=0' ) ) ); ?>" rel="bookmark"><?php the_title(); ?></a>
			</h1>
			<?php endif; // is_single() ?>
			<?php if ( comments_open() ) : ?>
				<div class="comments-link">
					<?php comments_popup_link( '<span class="leave-reply">' . _( 'Leave a reply' ) . '</span>', _( '1 Reply' ), _( '% Replies' ) ); ?>
				</div><!-- .comments-link -->
		...
		<div class="entry-content">
			<?php the_post_thumbnail(); ?>
		        <?php the_content( _( 'Continue reading <span class="meta-nav">&rarr;</span>' ) ); ?>
			<?php wp_link_pages( array( 'before' => '<div class="page-links">' . _( 'Pages:' ), 'after' => '</div>' ) ); ?>
